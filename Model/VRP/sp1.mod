...
var d integer >=0;				#atraso con que se llega al nodo L con respecto al fin de la jornada (debia ser TW)
...
#SP.
minimize SubProb: delayCost*d + distanceCost*sum{l in 1..Lcap-1} t[l] - sum{i in I2} pi[i]*a[i] -mu;
#minimize SubProb: delayCost*d + distanceCost*sum{l in 1..Lcap-1} t[l];

#Toma degenerancia
#subject to aux2: delayCost*d + distanceCost*sum{l in 1..Lcap-1} t[l] - sum{i in I2} pi[i]*a[i] -mu <= 0;
...
#res 18:
subject to timing3: exists {i in nodes} s[Lcap] = i and d = max(w[Lcap]+service[i]-480,0);
subject to timing4 {l in 2..Lcap}: w[l] <= 480;
